...
export function InitiativeFilters({ active, onChange }: { active: Direction | 'all'; onChange: (direction: Direction | 'all') => void }) {
  const { directions, initiatives } = useCatalog();
  return <div className="filters" aria-label="Фильтр по направлению"><button aria-pressed={active==='all'} onClick={()=>onChange('all')}>Все <span>{initiatives.length}</span></button>{directions.map(d=>{const Icon=directionIcons[d.id]; return <button key={d.id} aria-pressed={active===d.id} onClick={()=>onChange(d.id)}><Icon size={16}/>{d.name}</button>;})}</div>;
}
export function InitiativeCard({ initiative, decision, target, spent, count, onTarget, onToggle }: {
  ...
  const { directions, districts, budget: BUDGET, required_decisions: required } = useCatalog();
  const selected = !!decision;
  const reason = !selected ? count>=required ? `Выбрано ${required} мероприятий` : spent+initiative.cost>BUDGET ? 'Недостаточно бюджета' : initiative.type==='District' && !target ? 'Сначала выберите район' : '' : '';
...
export function ScenarioBar({ spent, count, onClear, onSimulate, loading = false }: { spent: number; count: number; onClear: () => void; onSimulate: () => void; loading?: boolean }) {
  const { budget: BUDGET, required_decisions: required } = useCatalog();
  return <section className="scenario-bar" aria-label="Ваш сценарий"><div className="scenario-title"><span className="live-dot"/><strong>Ваш сценарий</strong><small>{count===required?'Готово к проверке':`Выберите ещё ${required-count}`}</small></div><div className="scenario-stat"><small>Бюджет</small><strong>{spent}<span> / {BUDGET}</span></strong></div><div className="scenario-stat"><small>Решения</small><strong>{count}<span> / {required}</span></strong></div><div className="scenario-stat remaining"><small>Осталось</small><strong>{BUDGET-spent}</strong></div><button className="clear-button" onClick={onClear} disabled={!count}><RotateCcw size={16}/>Очистить</button><button className="primary" disabled={count!==required || loading} onClick={onSimulate}>{loading ? 'Расчёт…' : 'Симулировать'}<ArrowRight size={17}/></button></section>;
}
